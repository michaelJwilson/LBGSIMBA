...
def get_simba(tracer, redshift, space=''):
    fpath           = '/home/mjwilson/LBGSIMBA/bigdat/simba_{}{}pos_{:.5f}.fits'.format(tracer, space, redshift)

    print('Solving for {}.'.format(fpath))
...
def calc_xi(test, boxsize, redshift, tracer, space):    
    # comoving Mpc/h.                                                                                                                                                                    
    pos         =  get_simba(tracer, redshift, space).astype(np.float32)
    ...
    ##  Save result:
    np.savetxt('dat/corrfunc3dxi_{}_{}_{:.3f}.txt'.format(tracer, space, redshift), np.c_[rps, pis, xi], fmt='%.6le')
