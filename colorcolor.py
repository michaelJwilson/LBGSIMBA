...
wave, three, ids3    =  get_pyloser(boxsize, 3.003070, nrows=nrows, nodust=nodust, root=root)
...
if maglim == True:
  ...
  #maglim5    =  five['LSST_z'].values < 25.8
   ...
   #maglim2 = np.ones_like(two['LSST_u'].values,   dtype=bool)
   maglim3  = np.ones_like(three['LSST_u'].values, dtype=bool)
   #maglim4 = np.ones_like(four['LSST_u'].values,  dtype=bool) 
   #maglim5 = np.ones_like(five['LSST_u'].values,  dtype=bool),\
...
pl.savefig('plots/colorcolor_maglim_{:d}_nodust_{:d}_gaussian_ebv_threetenths.png'.format(maglim, nodust))
