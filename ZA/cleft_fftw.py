...
class CLEFT:
    ...
    def combine_bias_terms_pk(self, bvec):
        ...
        So bvec = [b1, b2, bs, alpha, 0, 0 ,0], where the three zeros are counterterms for the velocity statistics
        
        '''
        arr = self.pktable
        
        b1, b2, bs, alpha, alpha_v, alpha_s, alpha_s2 = bvec # only alpha is relevant here
        bias_monomials = np.array([1,1,1, b1, b1**2, b2, b1*b2, b2**2, bs, b1*bs, b2*bs, bs**2])

        kv = arr[:,0]; za = arr[:,1]
        pktemp = np.copy(arr)[:,1:]

        res = np.sum(pktemp * bias_monomials, axis =1) + alpha*kv**2 * za
